...
def inference(
    ...
    if cfg.TEST.RE_RANKING == 'no':
        ...
        if cfg.MODEL.Evaluate == "ClothChangingSetting":
            evaluator = create_supervised_evaluator(model, metrics={
                'r1_mAP_longterm': R1_mAP_longterm(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                     device=device)
        ...
        if cfg.MODEL.Evaluate == "ClothChangingSetting":
            evaluator = create_supervised_evaluator(model, metrics={
                'r1_mAP_longterm': R1_mAP_reranking_longterm(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                     device=device)

        elif cfg.MODEL.Evaluate == "StandardSetting":
            evaluator = create_supervised_evaluator(model, metrics={
                'r1_mAP': R1_mAP_reranking(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)}, device=device)

        elif cfg.MODEL.Evaluate == "both":
            evaluator1 = create_supervised_evaluator(model, metrics={
                'r1_mAP_longterm': R1_mAP_reranking_longterm(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                     device=device)
            evaluator2 = create_supervised_evaluator(model, metrics={
                'r1_mAP': R1_mAP_reranking(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)}, device=device)
            evaluator = (evaluator1, evaluator2)
        else:
            raise ValueError('Only support for \'ClothChangingSetting\' and \'StandardSetting\' and \'both\', but got {}'.format(
                    cfg.MODEL.Evaluate))
    ...
    if cfg.MODEL.Evaluate == "ClothChangingSetting":
        ...
        CC_cmc, CC_mAP = evaluator.state.metrics['r1_mAP_longterm']
        ...
        return CC_cmc, CC_mAP, None, None
    elif cfg.MODEL.Evaluate == "StandardSetting":
        evaluator.run(val_loader)
        SS_cmc, SS_mAP = evaluator.state.metrics['r1_mAP']
        ...
        return None, None, SS_cmc, SS_mAP
